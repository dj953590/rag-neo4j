...
the text in markdown format as well.
...
def to_markdown(
    ...
    def ocr_to_markdown(data: OnnxtrDocument) -> str:
        """Convert OCR data to markdown format."""
        markdown_lines = []
        ...
        for page_idx, page in enumerate(data.pages):
            ...
            # Join all lines and remove trailing whitespace
...
if __name__ == "__main__":
    ...
    doc_name = "citibank-scanned"
    filename = (
            Path(__file__).parent.parent.parent / 'docs' / (doc_name + ".pdf")
    )  # Replace with your PDF file path

    md = MarkItDown(enable_plugins=False) # Set to True to enable plugins
    result = md.convert(str(filename), extract_pages=True)  # Convert the PDF to Markdown
    ...
    extract_words=False,
    md_data = to_markdown(doc, pages=pages, page_chunks=True, extract_words=True, embed_images=True,)
