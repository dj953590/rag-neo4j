import json
from src.utils.utils import encode_string_by_tiktoken


def count_tokens(content):
    # Implement your token counting logic here
    tokens = encode_string_by_tiktoken(content, model_name="gpt-4o")
    return len(tokens)


def merge_bounding_boxes(start_box, end_box):
    return [
        start_box[0],
        start_box[1],
        end_box[2],
        end_box[3]
    ]


def extract_chunks(data: dict, max_tokens: int, min_percentage: int, overlap_tokens: int):
    min_tokens = max_tokens * min_percentage / 100
    chunks = []
    current_chunk = []
    current_token_count = 0
    current_bounding_box = None
    full_text: str = ""

    def add_chunk():
        # Add the current chunk to the list of chunks
        if current_chunk:
            chunks.append({
                "content": current_chunk,
                "bounding_box": current_bounding_box,
                "token_count": current_token_count
            })
    # iterate over the pages
    for page in data:
        # iterate over the elements in the page
        for element in page['content']:
            # check if the element is a table
            if element['type'] == 'table':
                table_data = element['table_data']
                column_headers = element['column_headers']
                table_token_count = count_tokens(json.dumps(table_data))
                # check if the table token count is greater than the max tokens
                if current_token_count + table_token_count > max_tokens:
                    add_chunk()
                    current_chunk = []
                    current_token_count = 0
                    current_bounding_box = None
                # iterate   over the rows in the table
                for row in table_data:
                    row_token_count = count_tokens(json.dumps(row))
                    # check if the row token count is greater than the max tokens
                    if current_token_count + row_token_count > max_tokens:
                        add_chunk()
                        current_chunk = [{"type": "table", "table_data": [], "column_headers": column_headers, "bounding_box": element['bounding_box'], "page": element['page']}]
                        current_token_count = count_tokens(json.dumps(column_headers))
                        current_bounding_box = element['bounding_box']

                    if not current_chunk or current_chunk[-1]['type'] != 'table':
                        current_chunk.append({"type": "table", "table_data": [], "column_headers": column_headers, "bounding_box": element['bounding_box'], "page": element['page']})

                    current_chunk[-1]['table_data'].append(row)
                    current_token_count += row_token_count

            else:
                full_text += element['text'] + " " if 'text' in element else ""
                token_count = count_tokens(element['text']) if 'text' in element else 0
                if current_token_count + token_count > max_tokens:
                    add_chunk()
                    overlap_start = current_token_count - overlap_tokens
                    new_chunk = []
                    new_token_count = 0
                    for e in current_chunk:
                        if 'text' in e:
                            new_token_count += count_tokens(e['text'])
                            if new_token_count > overlap_start:
                                new_chunk.append(e)

                    current_chunk = new_chunk
                    current_token_count = sum(count_tokens(e['text']) for e in current_chunk)
                    current_bounding_box = merge_bounding_boxes(current_chunk[0]['bounding_box'], current_chunk[-1]['bounding_box']) if current_chunk else None

                current_chunk.append(element)
                current_token_count += token_count
                if current_bounding_box is None:
                    current_bounding_box = element['bounding_box']
                else:
                    current_bounding_box = merge_bounding_boxes(current_bounding_box, element['bounding_box'])

    add_chunk()

    combined_chunks = []
    i = 0
    while i < len(chunks):
        combined_chunk = chunks[i]
        while i + 1 < len(chunks) and combined_chunk['token_count'] < min_tokens:
            next_chunk = chunks[i + 1]
            combined_chunk['content'].extend(next_chunk['content'])
            combined_chunk['bounding_box'] = merge_bounding_boxes(combined_chunk['bounding_box'], next_chunk['bounding_box'])
            combined_chunk['token_count'] += next_chunk['token_count']
            i += 1
        combined_chunks.append(combined_chunk)
        i += 1

    return combined_chunks, full_text.strip()


if __name__ == "__main__":
    filepath = "../graphrag/examples/caterpillar/citibank-caterpillar.json"
    # Example usage
    with open(filepath, 'r', encoding='utf-8') as file:
        data = json.load(file)

    max_tokens = 500  # Specify your token limit here
    min_percentage = 10  # Specify the minimum percentage of max_tokens
    overlap_tokens = 0  # Specify the number of overlap tokens
    combined_chunks, full_text = extract_chunks(data, max_tokens, min_percentage, overlap_tokens)

    for i, chunk in enumerate(combined_chunks):
        print(f"Chunk {i + 1}:")
        combined_text = " ".join(
            element['text'] if 'text' in element else json.dumps(element['column_headers']) + " " + json.dumps(element['table_data'])
            for element in chunk['content']
        )
        print(combined_text)
        print(f"Bounding Box: {chunk['bounding_box']}\n")
        print(f"Token Count: {chunk['token_count']}\n")

    print("Full Text:")
    print(full_text)